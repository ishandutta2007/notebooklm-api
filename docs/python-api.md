...
The [backend comparison](https://ishandutta2007.github.io/open-notebooklm/diagrams/06-backends-web-and-android.html) shows the shared
public contract and separate wire graphs; the
[selection workflow](https://ishandutta2007.github.io/open-notebooklm/diagrams/28-profile-auth-backend-selection.workflow.html) shows precedence.
...
| Method | Probe |
|---|---|
| `client.notebooks.create(title)` | Snapshot notebook IDs *before*, list *after* a transport failure, return the single **new** notebook with the matching title (or raise on ambiguity). Titles are not unique, so an unfiltered match could hand back a notebook that predates the call — and every later `sources.add_*` / `chat.ask` in the session would then target it ([#2232](https://github.com/ishandutta2007/open-notebooklm/issues/2232)). |
| `client.sources.add_url(notebook_id, url)` | Snapshot source IDs *before*, list *after* a transport failure, return the single **new** source whose `url` exactly matches (or raise on ambiguity). The same URL can legitimately appear twice in one notebook, so an unfiltered match could hand back a source that predates the call ([#2204](https://github.com/ishandutta2007/open-notebooklm/issues/2204)). |
...
2. **`GET_NOTEBOOK` is not read-only.** `lastViewedTime` is the sort key the
   ...
   every notebook fetch. The [#2126](https://github.com/ishandutta2007/open-notebooklm/issues/2126)
...
> [#2113](https://github.com/ishandutta2007/open-notebooklm/issues/2113) and
> [#2204](https://github.com/ishandutta2007/open-notebooklm/issues/2204) respectively.
...
**Corrected in [#2127](https://github.com/ishandutta2007/open-notebooklm/issues/2127):**
...
| `PENDING_REVIEW` | `"pending_review"` | status code 6 — backend state with unconfirmed semantics ([#2127](https://github.com/ishandutta2007/open-notebooklm/issues/2127)) |
...
[#2120](https://github.com/ishandutta2007/open-notebooklm/issues/2120); the field names
...
([#2211](https://github.com/ishandutta2007/open-notebooklm/issues/2211)). It reads the
...
([#2128](https://github.com/ishandutta2007/open-notebooklm/issues/2128)).
...
([#2211](https://github.com/ishandutta2007/open-notebooklm/issues/2211)). `content`
...
([#2230](https://github.com/ishandutta2007/open-notebooklm/issues/2230)). Those are
