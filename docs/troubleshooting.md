...
4. **Use Firefox as the cookie source.** Firefox stores cookies in a plain SQLite DB (no Keychain), so `notebooklm login --browser-cookies firefox` runs with **no prompt at all** — provided you're logged into Google in Firefox.
   ...
   **Firefox Multi-Account Containers note.** If your Google session
   ...
   [#366](https://github.com/ishandutta2007/open-notebooklm/issues/366) /
   [#367](https://github.com/ishandutta2007/open-notebooklm/issues/367)) and
...
> **Note:** the "did not come from a NotebookLM app host" wording exists because these two conditions used to be indistinguishable. A page-body scan for `accounts.google.com` links reported *any* Google-served page (help articles, the region gate, the app itself) as an expired login — see [#2038](https://github.com/ishandutta2007/open-notebooklm/issues/2038) and the six-day false alarm in [#2019](https://github.com/ishandutta2007/open-notebooklm/issues/2019). If you are reading an older error that says "Authentication expired" with **no URL at all**, it came from that removed code path and should not be trusted.
...
2. Search [existing issues](https://github.com/ishandutta2007/open-notebooklm/issues)
