...
The [selection workflow](https://ishandutta2007.github.io/open-notebooklm/diagrams/28-profile-auth-backend-selection.workflow.html) summarizes the
...
npx skills add ishandutta2007/open-notebooklm
```
...
> **Production deployment patterns (tracked in [#417](https://github.com/ishandutta2007/open-notebooklm/issues/417)).** Production-grade FastAPI/Django integration — client lifetime in a `lifespan` handler, httpx pool sizing, behavior under concurrent CSRF refresh, multi-tenant `storage_state.json` rotation, a service-shaped Dockerfile, and structured rate-limit/backoff patterns — is not yet covered in `docs/python-api.md`. These were intentionally deferred from the install-docs consolidation (PR #416) to keep its scope focused. See [#417](https://github.com/ishandutta2007/open-notebooklm/issues/417) for the gap inventory and acceptance criteria.
...
```bash
git clone https://github.com/ishandutta2007/open-notebooklm.git
cd notebooklm-py
...
The [REST subsystem diagram](https://ishandutta2007.github.io/open-notebooklm/diagrams/18-rest-server-subsystem.html) shows its
...
- the one-click `.mcpb` desktop bundle — download it from the [latest release](https://github.com/ishandutta2007/open-notebooklm/releases/latest) (**Assets**) and use Claude Desktop's "Install Extension". Each stable release attaches a prebuilt, version-matched bundle; see [`desktop-extension/README.md`](../desktop-extension/README.md).
