...
The [CLI subsystem diagram](https://ishandutta2007.github.io/open-notebooklm/diagrams/16-cli-subsystem.html) explains how commands,
services, the transport-neutral application layer, and output rendering fit together. The
[selection workflow](https://ishandutta2007.github.io/open-notebooklm/diagrams/28-profile-auth-backend-selection.workflow.html) shows how profile,
...
| `ask --save-as-note` | Save response as a note. When the answer contains `[N]` citations, the saved note preserves interactive hover-anchored citation links matching the NotebookLM web UI's "Save to note" behavior ([issue #660](https://github.com/ishandutta2007/open-notebooklm/issues/660)). Answers without citations fall back to a plain-text note. | `notebooklm ask "Explain X" --save-as-note` |
...
Rows that are genuinely mid-upload also report `preparing`, so this filter cannot by itself tell "abandoned" from "in flight" — re-run it a minute apart and act only on rows that persist. Nothing is deleted automatically; that posture is deliberate (see [#2110](https://github.com/ishandutta2007/open-notebooklm/issues/2110)).
...
The packaged wheel includes the repo-root `SKILL.md`, so the same skill content powers `notebooklm skill install`, GitHub discovery, and `npx skills add ishandutta2007/open-notebooklm`.
